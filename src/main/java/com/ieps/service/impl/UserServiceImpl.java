...
public class UserServiceImpl implements UserService {
    ...
    public ServerResponse login(String userNum, String userPwd) {
        User user = userMapper.login(userNum, EncryptUtil.AESencode(userPwd, Const.PWD_KEY));
    ...
    public ServerResponse<User> getMenu(String userNum) {
        ...
        return ServerResponse.createBySuccess("加载菜单完成！", user);
    }
    ...
    public ServerResponse register(User user, UserInfo userInfo, Integer roleId) {
        ...
        // 使用AES加密算法经行加密
        user.setUserPwd(EncryptUtil.AESencode(user.getUserPwd(), Const.PWD_KEY));
    ...
    public ServerResponse checkUserPwdWithUserNum(String userNum, String userPwd) {
        ...
        if (userMapper.selectUserPwdWithUserNum(userNum, EncryptUtil.AESencode(userPwd, Const.PWD_KEY)) <= 0) {
    ...
    public ServerResponse forgetPwd(String userNum, String userPwd) {
        ...
        if (userMapper.updatePwd(userNum, EncryptUtil.AESencode(userPwd, Const.PWD_KEY)) < 1) {
    ...
    public ServerResponse modifyPwd(User user) {
        // 使用AES加密算法经行加密
        user.setUserPwd(EncryptUtil.AESencode(user.getUserPwd(), Const.PWD_KEY));
        user.setNewPassword(EncryptUtil.AESencode(user.getNewPassword(), Const.PWD_KEY));
        user.setRePassword(EncryptUtil.AESencode(user.getRePassword(), Const.PWD_KEY));
    ...
    public ServerResponse getUserPwdWithUserNum(String userNum) {
        ...
        user.setUserPwd(EncryptUtil.AESdecode(user.getUserPwd(), Const.PWD_KEY));
